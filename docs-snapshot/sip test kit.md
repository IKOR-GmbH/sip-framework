...
- [Test Kit provides](#Test Kit provides)
...
It will provide default behavior and mock all external endpoints.
Adding specific behavior for each endpoint can be done in test case definition in 'with-mocks' section

## Endpoint reports

Test Kit will provide reports for all mocked endpoints, both with default and user defined behavior.
For each test report in the 'Endpoints' section there will be an overview of request that each endpoint received.
...
To be able to test your adapter with SIP Test Kit, the following configuration is needed:
```yaml
sip:
  testkit:
    enabled: true
...
Then, a test class needs to be created inside test package of the adapter, which extends SIPBatchTest. 
...
The third step is to provide the TestCaseDefinition file in yaml format in the `test/resources` package
...
You also need to provide info to the test framework about the location of your TestCaseDefinition file by setting the
following property:
...
In this section a request that should be sent to an adapter is defined.
...
Body and header validation are possible by either defining regex (Java) expression or matching exact String content.

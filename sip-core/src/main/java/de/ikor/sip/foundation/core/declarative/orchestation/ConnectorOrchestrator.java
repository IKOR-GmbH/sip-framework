...
public class ConnectorOrchestrator implements Orchestrator<ConnectorOrchestrationInfo> {
    ...
    private void defaultRequestTransformer(final RouteDefinition definition) {
        definition.log(LoggingLevel.WARN, String.format("No request transformation definition has been assigned to connector '%s' in connector-class '%s'", relatedConnector.getConnectorId(), relatedConnector.getClass().getName()));
    }

    private void defaultResponseTransformer(final RouteDefinition definition) {
        definition.log(LoggingLevel.WARN, String.format("No response transformation definition has been assigned to connector '%s' in connector-class '%s'", relatedConnector.getConnectorId(), relatedConnector.getClass().getName()));
    }

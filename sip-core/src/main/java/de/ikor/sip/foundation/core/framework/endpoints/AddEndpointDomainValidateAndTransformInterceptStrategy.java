...
@Component
public class AddEndpointDomainValidateAndTransformInterceptStrategy
    implements InterceptStrategy, Ordered {
  ...
  private void doWrappedProcessing(Processor target, Exchange exchange) {
    ...
    endpoint
        ...
        .ifPresent(
            domainClassType ->
                domainValidationProcessing(domainClassType, exchange, endpoint.getEndpointUri()));
    endpoint
        .getTransformFunction()
        .ifPresent(function -> domainTransformationProcessing(function, exchange));
  ...
  private boolean isInMemoryComponent(Processor target) {
    return StringUtils.startsWithAny(
        ((EndpointAware) target).getEndpoint().getEndpointUri(), NON_OUTGOING_PROCESSOR_PREFIXES);
  }

  @SneakyThrows
  private void domainValidationProcessing(
      Class<?> domainClassType, Exchange exchange, String endpointUri) {

...
/**
 * {@link ProcessorProxyMockRegistry} is a register which collects mock functions for corresponding processor proxies.
 * Register is used for collecting only mock functions registered during runtime.
 */
@Getter
@Component
public class ProcessorProxyMockRegistry {
   ...
   *
   * @param processorId processor id used for matching with the ProxyProcessor {@link ProcessorProxy}.
  ...
  public void registerMock(String processorId, UnaryOperator<Exchange> mockFunction) {
    if (processorId == null || proxyMocks.containsKey(processorId)) {
      throw new BadProxyProcessorIdException("Registering mock with processor id which is null or already exists");
    }

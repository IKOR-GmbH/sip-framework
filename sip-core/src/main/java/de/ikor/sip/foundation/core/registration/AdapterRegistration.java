...
class AdapterRegistration implements InitializingBean, DisposableBean {
  ...
  public void afterPropertiesSet() {
    log.info(
        "Adapter registration for {} with id {} is starting",
        this.properties.getAdapterName(),
        this.properties.getInstanceId());
    this.telemetryDataCollector.initialize(
        this.properties.getInstanceId(),
        this.properties.getAdapterName(),
        this.properties.getInstanceUri(),
        this.properties.getInterval());
    this.registrationScheduler.startScheduler(
        () ->
            this.registrationWebClient.sendPostRequest(
                "/register", this.telemetryDataCollector.collectData()));
  }
  ...
  public void destroy() {
    log.info(
        "Adapter registration for {} with id {} is stopping",
        this.properties.getAdapterName(),
        this.properties.getInstanceId());
    this.registrationScheduler.stopScheduler();
    this.registrationWebClient.sendDeleteRequest(
        String.format("/deregister/%s", this.properties.getInstanceId()));
  }
}

...
class ProcessorProxyMockSetupTest {
  ...
  void setUp() {
    ...
    processorProxyMockSetup =
        new ProcessorProxyMockSetup(proxyRegistry, processorProxyMockRegistry);
    setUpProcessorProxy();
  }
  ...
  void mockProcessorProxys() {
    ...
    assertThat(proxyRegistry.getProxy(PROCESSOR_ID).get().getMockFunction())

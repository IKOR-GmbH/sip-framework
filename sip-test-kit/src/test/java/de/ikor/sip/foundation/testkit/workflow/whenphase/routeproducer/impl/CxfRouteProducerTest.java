...
class CxfRouteProducerTest {
    ...
    private static final String RESPONSE_BODY = "<soap:Envelope xmlns:soap=\"http://schemas.xmlsoap.org/soap/envelope/\"><soap:Body><ns2:AddBookResponse xmlns:ns2=\"http://www.cleverbuilder.com/BookService/\"><ns2:Book><ID>1</ID><Title>Camel in Action</Title><Author>Claus Ibsen</Author></ns2:Book></ns2:AddBookResponse></soap:Body></soap:Envelope>";
    ...
    @Test
    void GIVEN_emptyRequest_WHEN_executeTask_THEN_validateGoodResponse() {
        // arrange
        ...
        ResponseEntity<String> routeExpectedResponse = new ResponseEntity<>(RESPONSE_BODY, HttpStatus.OK);
        when(restTemplate.exchange(anyString(), any(HttpMethod.class), any(),
                ArgumentMatchers.<ParameterizedTypeReference>any())).thenReturn(routeExpectedResponse);

        // act
        Exchange actualExchange = spySubject.executeTask(exchange, endpoint);

        // assert
        assertThat(actualExchange.getMessage().getBody()).isEqualTo(routeExpectedResponse.getBody());
    }

    private Exchange createExchange(String body, Map<String, Object> headers) {
        ExchangeBuilder exchangeBuilder =
                ExchangeBuilder.anExchange(camelContext).withBody(body);
